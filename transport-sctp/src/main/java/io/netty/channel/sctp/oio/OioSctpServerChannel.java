...
public class OioSctpServerChannel extends AbstractOioMessageChannel
    ...
    private static SctpServerChannel newServerSocket() {
        try {
            ...
            throw new ChannelException("failed to create a sctp httpserver channel", e);
        }
    }
    ...
    public OioSctpServerChannel(SctpServerChannel sch) {
        super(null);
        if (sch == null) {
            throw new NullPointerException("sctp httpserver channel");
        }
        ...
        try {
            ...
            throw new ChannelException("failed to initialize a sctp httpserver channel", e);
        } finally {
            if (!success) {
                try {
                    sch.close();
                } catch (IOException e) {
                    logger.warn("Failed to close a sctp httpserver channel.", e);
                }
            }
        }
    }
