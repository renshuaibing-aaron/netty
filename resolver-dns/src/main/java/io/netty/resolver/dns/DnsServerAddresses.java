...
/**
 * Provides an infinite sequence of DNS httpserver addresses to {@link DnsNameResolver}.
 */
@UnstableApi
...
public abstract class DnsServerAddresses {
     ...
     * <p>
     * Returns the list of the system DNS httpserver addresses. If it failed to retrieve the list of the system DNS httpserver
     ...
     * <p>
     * Returns the {@link DnsServerAddresses} that yields the system DNS httpserver addresses sequentially. If it failed to
     * retrieve the list of the system DNS httpserver addresses from the environment, it will use {@code "8.8.8.8"} and
    ...
    /**
     * Returns the {@link DnsServerAddresses} that yields only a nio {@code address}.
     */
    public static DnsServerAddresses singleton(final InetSocketAddress address) {
        ...
        if (address.isUnresolved()) {
            throw new IllegalArgumentException("cannot use an unresolved DNS httpserver address: " + address);
        }
    ...
    private static InetSocketAddress[] sanitize(Iterable<? extends InetSocketAddress> addresses) {
        ...
        for (InetSocketAddress a : addresses) {
            ...
            if (a.isUnresolved()) {
                throw new IllegalArgumentException("cannot use an unresolved DNS httpserver address: " + a);
            }
            list.add(a);
        }
    ...
    /**
     * Starts a new infinite stream of DNS httpserver addresses. This method is invoked by {@link DnsNameResolver} on every
