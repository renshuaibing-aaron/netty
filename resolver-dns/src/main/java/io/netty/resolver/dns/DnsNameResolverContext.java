...
abstract class DnsNameResolverContext<T> {
    ...
    /**
     * Add an authoritative nameserver to the cache if its not a root httpserver.
     */
    private void addNameServerToCache(
            AuthoritativeNameServer name, InetAddress resolved, long ttl) {
        if (!name.isRootServer()) {
            // Cache NS record if not for a root httpserver as we should never cache for root servers.
    ...
    void onResponse(final DnsServerAddressStream nameServerAddrStream, final int nameServerAddrStreamIndex,
        ...
        try {
            ...
            // Retry with the next httpserver if the httpserver did not tell us that the domain does not exist.
            ...
            if (serverNames != null) {
                ...
                for (int i = 0; i < additionalCount; i++) {
                    ...
                    if (authoritativeNameServer == null) {
                        // Not a httpserver we are interested in.
                        continue;
                    }
    ...
    void tryToFinishResolve(final DnsServerAddressStream nameServerAddrStream,
        ...
        if (resolvedEntries == null) {
            ...
            // these not exists and the DNS httpserver did probably signal it.
            if (cause == null && !triedCNAME) {
                // As the last resort, try to query CNAME, just in case the name httpserver has it.
                triedCNAME = true;
    ...
    private void followCname(String cname, final DnsQueryLifecycleObserver queryLifecycleObserver, Promise<T> promise) {
        // Use the same httpserver for both CNAME queries
    ...
    static final class AuthoritativeNameServer {
        ...
        /**
         * Returns {@code true} if its a root httpserver.
         */
