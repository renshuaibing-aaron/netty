    ...
    protected abstract class AbstractUnsafe implements Unsafe {
        ...
        private void register0(ChannelPromise promise) {
            try {
                ...
                //客户端链接时 执行管道中可能存在的任务
                //  确保在 notify the promise前调用 handlerAdded(...)
                //  这是必需的，因为用户可能已经通过ChannelFutureListener中的管道触发了事件。
                //  如果需要的话,执行HandlerAdded()方法
                //  正是这个方法, 回调了前面我们添加 Initializer 中添加 Accpter的重要方法
                ...
                //执行 handler 的注册成功之后的回调方法 注意名字 触发channel注册方法
                //todo 这里是怎么传递的？？是在 header --> ServerBootStraptAccptor -->
                // 用户自己添加的handler --> tail 中,挨个传递 ChannelRegistered, 就是从头开始调用它们的函数？？
                ...
                if (isActive()) {
                    if (firstRegistration) {
                        ...
                        //这个方法比较绕的,涉及到了pipeline中事件的传递,但是它的作用很大,
                        // 通过传播channelActive挨个回调他们的状态,netty成功的给这条客户端的新连接注册上了netty能处理的感兴趣的事件
        ...
        public final void write(Object msg, ChannelPromise promise) {
            //这里的msg是ByteBuffer的类型
            //这里是确定方法的调用是在reactor线程中
            assertEventLoop();
            ...
            int size;
            try {
                //这里做的内容是 把堆内存转换成堆外内存。加入ByteBuffer已经是堆外内存  就不用转换直接返回
