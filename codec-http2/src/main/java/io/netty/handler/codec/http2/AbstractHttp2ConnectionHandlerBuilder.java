...
public abstract class AbstractHttp2ConnectionHandlerBuilder<T extends Http2ConnectionHandler,
    ...
    // The property that will prohibit connection() and codec() if set by httpserver(),
    // because this property is used only when this builder creates a Http2Connection.
    private Boolean isServer;
    private Integer maxReservedStreams;

    // The property that will prohibit httpserver() and codec() if set by connection().
    private Http2Connection connection;

    // The properties that will prohibit httpserver() and connection() if set by codec().
    private Http2ConnectionDecoder decoder;
    private Http2ConnectionEncoder encoder;

    // The properties that are:
    // * mutually exclusive against codec() and
    // * OK to use with httpserver() and connection()
    ...
    /**
     * Returns if {@link #build()} will to create a {@link Http2Connection} in httpserver mode ({@code true})
    ...
    /**
     * Sets if {@link #build()} will to create a {@link Http2Connection} in httpserver mode ({@code true})
     * or client mode ({@code false}).
     */
    protected B server(boolean isServer) {
        enforceConstraint("httpserver", "connection", connection);
        enforceConstraint("httpserver", "codec", decoder);
        enforceConstraint("httpserver", "codec", encoder);
     ...
     * <p>
     * By default this value will be ignored on the httpserver for local endpoint. This is because the RFC provides
    ...
    protected B connection(Http2Connection connection) {
        ...
        enforceConstraint("connection", "httpserver", isServer);
    ...
    protected B codec(Http2ConnectionDecoder decoder, Http2ConnectionEncoder encoder) {
        enforceConstraint("codec", "httpserver", isServer);
    ...
    private void enforceNonCodecConstraints(String rejected) {
        enforceConstraint(rejected, "httpserver/connection", decoder);
        enforceConstraint(rejected, "httpserver/connection", encoder);
    }
