...
public class Http2ConnectionHandler extends ByteToMessageDecoder implements Http2LifecycleManager,
    ...
    public void onHttpClientUpgrade() throws Http2Exception {
        if (connection().isServer()) {
            throw connectionError(PROTOCOL_ERROR, "Client-side HTTP upgrade requested for a httpserver");
        }
        if (!prefaceSent()) {
    ...
    /**
     * Handles the httpserver-side (cleartext) upgrade from HTTP to HTTP/2.
    ...
    private final class PrefaceDecoder extends BaseDecoder {
        ...
        private void sendPreface(ChannelHandlerContext ctx) throws Exception {
            ...
            // Both client and httpserver must send their initial settings.
    ...
    /**
     * Notifies client that this httpserver has received headers that are larger than what it is
