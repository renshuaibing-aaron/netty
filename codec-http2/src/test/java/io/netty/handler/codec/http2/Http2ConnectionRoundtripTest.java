...
public class Http2ConnectionRoundtripTest {
    ...
    public void inflightFrameAfterStreamResetShouldNotMakeConnectionUnusable() throws Exception {
        ...
        // Create a nio stream by sending a HEADERS frame to the httpserver.
    ...
    public void headersWithEndStreamShouldNotSendError() throws Exception {
        ...
        // The httpserver will not respond, and so don't wait for graceful shutdown
    ...
    public void encodeViolatesMaxHeaderListSizeCanStillUseConnection() throws Exception {
        ...
        // Manually add a listener for when we receive the expected headers on the httpserver.
        doAnswer(new Answer<Void>() {
            @Override
            public Void answer(InvocationOnMock invocationOnMock) throws Throwable {
    ...
    public void testSettingsAckIsSentBeforeUsingFlowControl() throws Exception {
        ...
        // The httpserver initially reduces the connection flow control window to 0.
        runInChannel(serverConnectedChannel, new Http2Runnable() {
            @Override
            public void run() throws Http2Exception {
                http2Server.encoder().writeSettings(serverCtx(),
        ...
        // Now the httpserver opens up the connection window to allow the client to send the pending data.
        runInChannel(serverConnectedChannel, new Http2Runnable() {
            @Override
            public void run() throws Http2Exception {
                http2Server.encoder().writeSettings(serverCtx(),
    ...
    public void headersWriteForPeerStreamWhichWasResetShouldNotGoAway() throws Exception {
        ...
        // Now have the httpserver attempt to send a headers frame simulating some asynchronous work.
        runInChannel(serverConnectedChannel, new Http2Runnable() {
            @Override
            public void run() throws Http2Exception {
    ...
    public void http2ExceptionInPipelineShouldCloseConnection() throws Exception {
        ...
        // Wait for the httpserver to create the stream.
        assertTrue(serverSettingsAckLatch.await(DEFAULT_AWAIT_TIMEOUT_SECONDS, SECONDS));
        assertTrue(requestLatch.await(DEFAULT_AWAIT_TIMEOUT_SECONDS, SECONDS));
    ...
    public void noMoreStreamIdsShouldSendGoAway() throws Exception {
        ...
        // Don't wait for the httpserver to close streams
        http2Client.gracefulShutdownTimeoutMillis(0);
    ...
    public void flowControlProperlyChunksLargeMessage() throws Exception {
        ...
        try {
            ...
            // Don't wait for httpserver to close streams
            http2Client.gracefulShutdownTimeoutMillis(0);
            data.release();
            out.close();
        }
    ...
    public void stressTest() throws Exception {
        ...
        // Collect all the ping buffers as we receive them at the httpserver.
        ...
        // Collect all the data buffers as we receive them at the httpserver.
