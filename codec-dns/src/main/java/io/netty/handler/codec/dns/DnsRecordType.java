...
public class DnsRecordType implements Comparable<DnsRecordType> {
    ...
    /**
     * Name httpserver record RFC 1035 Delegates a DNS zone to use the given
     ...
     * httpserver, the email of the domain administrator, the domain serial number,
     ...
     * resource record is used to associate a TLS httpserver certificate or public
     ...
     * as coming from an approved recursive name httpserver similar to DNSSEC.
     */
     ...
     * authoritative httpserver is unable to fulfill the request due to
     ...
     * from the master name httpserver to secondary name servers.
     */
     ...
     * name httpserver. If the name httpserver does not have any information on the
     ...
     * the name httpserver has only the A record cached, only the A record will be
