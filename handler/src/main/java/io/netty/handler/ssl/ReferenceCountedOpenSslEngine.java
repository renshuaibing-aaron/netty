...
public class ReferenceCountedOpenSslEngine extends SSLEngine implements ReferenceCounted, ApplicationProtocolAccessor {
     ...
     *                             {@code false} allows for partial and/or multiple packets to be process in a nio
     *                             wrap or unwrap call.
    ...
    public void setOcspResponse(byte[] response) {
        ...
        if (clientMode) {
            throw new IllegalStateException("Not a httpserver SSLEngine");
        }
    ...
    /**
     * Returns the OCSP response or {@code null} if the httpserver didn't provide a stapled OCSP response.
     */
    @UnstableApi
    ...
    public final SSLEngineResult wrap(
        ...
        synchronized (this) {
            ...
            try {
                ...
                // Prepare OpenSSL to work in httpserver mode and receive handshake
                ...
                if (jdkCompatibilityMode) {
                    ...
                    // jdkCompatibilityMode will only produce a nio TLS packet, and we don't aggregate src buffers,
    ...
    public final SSLEngineResult unwrap(
        ...
        synchronized (this) {
            ...
            int packetLength;
            // The JDK implies that only a nio SSL packet should be processed per unwrap call [1]. If we are in
                        ...
                        for (;;) {
                            ...
                            if (bytesRead > 0) {
                                ...
                                if (!dst.hasRemaining()) {
                                    ...
                                    // a nio TLS packet and should stop consuming until this method is called again.
                                    break srcLoop;
                                }
                            } else {
    ...
    public final synchronized void beginHandshake() throws SSLException {
        switch (handshakeState) {
            ...
            case FINISHED:
                if (clientMode) {
                    // Only supported for httpserver mode at the moment.
                    throw RENEGOTIATION_UNSUPPORTED;
                }
                // For renegotiate on the httpserver side we need to issue the following command sequence with openssl:
                //
                // SSL_renegotiate(ssl)
    ...
    private final class OpenSslSession implements SSLSession {
        ...
        private void initPeerCerts() {
            ...
            if (clientMode) {
                ...
                // if used on the httpserver side SSL_get_peer_cert_chain(...) will not include the remote peer
